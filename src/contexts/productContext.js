...
export const ProductProvider = props => {
  ...
  const getOrderDetails = orderId => {
    return store.orders.find(order => order.orderNumber === 1585186479089);
  };
  ...
  const getPrice = id => {
    let product = store.products.filter(item => item.id === id)[0];
    if (product.salesPrice && product.regularPrice) {
      return (
        <p>
          <span
            className='price-onsale-regular'
            style={{ textDecoration: "line-through", marginRight: "5px" }}>
            ${product.regularPrice}
          </span>
          <span
            className='price-onsale-sale'
            style={{ fontWeight: "bold", color: "#008000" }}>
            ${product.salesPrice}
          </span>
        </p>
      );
    } else if (!product.salesPrice && product.regularPrice) {
      return (
        <p>
